...
TH1I *hist_adc_off[12]; 
 ...
 Float_t Qfull = 0.;
  ...
  t21->SetDirectory(0);
	...
	for (Int_t ii=0; ii <=19; ii++) {
	for (Int_t jj=0; jj <=19; jj++) {
	
	qqq.str("");
	qqq << "Pdiff_" << ii << "_" << jj;
	Pdiff[ii][jj] = new TH1F(qqq.str().c_str(),qqq.str().c_str(),200,-0.15,0.15);
	
	};
	};
	
	/*
	for (Int_t ww=0; ww <=5; ww++) {
	
	qqq.str("");
	qqq << "Wbefor_sector_" << i+1;
	Wbefor[ww] = new TH1F(qqq.str().c_str(),qqq.str().c_str(),100,0.8,1.2);
	
	qqq.str("");
	qqq << "Wafter_sector_" << i+1;
	Wafter[ww] = new TH1F(qqq.str().c_str(),qqq.str().c_str(),100,0.8,1.2);
...
//nachinaem vsegda so 2go blocka
    ...
    P_P = br_p->GetLeaf("p")->GetValue(0);
    th_P = (180./M_PI)*acos(br_cz->GetLeaf("cz")->GetValue(0));
...
//  cout << th_P << "  " << P_P << endl;
   ...
   Pdiff[int((P_P-0.1)/0.095)][int((th_P)/6.)]->Fill((P_P-(br_mcp->GetLeaf("mcp")->GetValue(0))),1.);
  
   
   };
   };
  //  cout << br_p->GetLeaf("p")->GetValue(0) << "  " << br_p->GetLeaf("p")->GetValue(1) << endl;
...
//   cout << br_dc->GetLeaf("dc")->GetValue(0) << "\n";
   ...
   P_EL = br_p->GetLeaf("p")->GetValue(0);
//   pdiff->Fill((br_mcp->GetLeaf("mcp")->GetValue(0)-P_EL)/(br_mcp->GetLeaf("mcp")->GetValue(0)),1.);
//   pdiff_vs_p->Fill((br_mcp->GetLeaf("mcp")->GetValue(0)),(br_mcp->GetLeaf("mcp")->GetValue(0)-P_EL),1.);
...
/*th_EL = (180./M_PI)*acos(br_cz->GetLeaf("cz")->GetValue(0));
  ...
  if ((P_EL>0.5)&&(P_EL<2.)) {
  if ((th_EL>20.)&&(th_EL<50.)) { 
  
   Pdiff[int((P_EL-0.5)/0.15)][int((th_EL-20.)/3.)]->Fill((P_EL-(br_mcp->GetLeaf("mcp")->GetValue(0))),1.);
...
//    cout << "l_time = " <<  br_t_l->GetLeaf("t_l")->GetValue(0)  << " block = " <<  block << "\n";
  ...
  block_last = block_last + block - 1;
     ...
     t21->Write("", TObject::kOverwrite);
      for (Int_t ii=0; ii <=19; ii++) {
	for (Int_t jj=0; jj <=19; jj++) {
     Pdiff[ii][jj]->Write("", TObject::kOverwrite);
     };
     };
     outFile->Write();
     outFile->Close();
