...
echo ' ' >> run_data$y.sh
@ z = $x + 1
#sed -e "1,$z d" file_list_data |  sed  -e "10q" | sed -e 's$/mss$jget /mss$g' | sed -e "s/.root/.root ./2">> run_data$y.sh
#echo ' ' >> run_data$y.sh
echo 'echo "1" > inp' >> run_data$y.sh
echo 'echo "2.039" >> inp' >> run_data$y.sh
echo 'echo "10" >> inp' >> run_data$y.sh
#sed -e "1,$z d" file_list_data |  sed  -e "10q" | sed -e 's$/mss/home/gleb/e1e/cooked_root/h10/$$g' | sed -e 's$clas$echo "clas$' | sed -e 's$root$root" >> inp$'>> run_data$y.sh
sed -e "1,$z d" file_list_data |  sed  -e "10q" | sed -e 's$/cache$echo "/cache$' | sed -e 's$cooked_root$qqq$' | sed -e 's$.root$.root" >> inp$' | sed -e 's$qqq$cooked_root$' >> run_data$y.sh
