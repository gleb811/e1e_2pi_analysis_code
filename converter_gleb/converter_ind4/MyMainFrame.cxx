...
//if (data_sim == 1) {
  ...
  br_dc_xec = t20->GetBranch("dc_xec");
  br_dc_yec = t20->GetBranch("dc_yec");
  br_dc_zec = t20->GetBranch("dc_zec");
...
//tsikl po sobitiyam/ mogno ispol'zovat' lyubuyu peremennuyu, tk v nih odinakovoe kol-vo sobitij 
  ...
  br_sc_t-> GetEntry(i);
  br_sc_r-> GetEntry(i);
  br_dc_xec -> GetEntry(i);
  br_dc_yec -> GetEntry(i);
  br_dc_zec -> GetEntry(i);
...
//v to vremia kak dlia dannih cikl po j srabativaet ne kazdi' raz, a tol'ko kogda nstart ne ravno nstop i tam mi uze vozvraschaemsia na skol'ko-to sobiti' nazad
 ...
 // br_tr_time-> GetEntry(j);
  ...
  br_dc_xec -> GetEntry(j);
  br_dc_yec -> GetEntry(j);
  br_dc_zec -> GetEntry(j);
 }; 
 ...
 flag = 0;
 if (((br_mcid->GetLeaf("mcid")->GetValue(4)) == 111)&&((br_mcnpart->GetLeaf("mcnpart")->GetValue()) > 4)){
  indtype = 3;
 flag = 1;
// cout << i << endl;
 };
...
//if (data_sim ==1){
dc_x_PIp = br_dc_xec->GetLeaf("dc_xec")->GetValue(br_dc->GetLeaf("dc")->GetValue(k)-1); 
dc_y_PIp = br_dc_yec->GetLeaf("dc_yec")->GetValue(br_dc->GetLeaf("dc")->GetValue(k)-1);
dc_z_PIp = br_dc_zec->GetLeaf("dc_zec")->GetValue(br_dc->GetLeaf("dc")->GetValue(k)-1);  
//};
...
//   if (data_sim == 1) 
//    cout << "Pbefore = " << P_P << " Pafter = " << P_P*(1. + corrfunc.correct_energy_theta_pf(P_P, th_P)) << endl;
    P_P = P_P*(1. + corrfunc.correct_energy_theta_pf(P_P, th_P));
...
//if (data_sim ==1){
dc_x_P = br_dc_xec->GetLeaf("dc_xec")->GetValue(br_dc->GetLeaf("dc")->GetValue(k)-1); 
dc_y_P = br_dc_yec->GetLeaf("dc_yec")->GetValue(br_dc->GetLeaf("dc")->GetValue(k)-1);
dc_z_P = br_dc_zec->GetLeaf("dc_zec")->GetValue(br_dc->GetLeaf("dc")->GetValue(k)-1);  
//};
...
//if (data_sim ==1){
dc_x_PIm = br_dc_xec->GetLeaf("dc_xec")->GetValue(br_dc->GetLeaf("dc")->GetValue(k)-1); 
dc_y_PIm = br_dc_yec->GetLeaf("dc_yec")->GetValue(br_dc->GetLeaf("dc")->GetValue(k)-1);
dc_z_PIm = br_dc_zec->GetLeaf("dc_zec")->GetValue(br_dc->GetLeaf("dc")->GetValue(k)-1);  
...
P_EL = P_EL_new - 0.0007;
th_EL = th_EL_new;
};	

if (data_sim == 2){
//cout << "P_EL = " << P_EL << " P_EL_corr = " << corrfunc.correct_pel_sim(P_EL,th_EL) << endl;
P_EL = corrfunc.correct_pel_sim(P_EL,th_EL);
};	
...
//if (data_sim ==1){
dc_x_EL = br_dc_xec->GetLeaf("dc_xec")->GetValue(br_dc->GetLeaf("dc")->GetValue(0)-1); 
dc_y_EL = br_dc_yec->GetLeaf("dc_yec")->GetValue(br_dc->GetLeaf("dc")->GetValue(0)-1);
dc_z_EL = br_dc_zec->GetLeaf("dc_zec")->GetValue(br_dc->GetLeaf("dc")->GetValue(0)-1);  
