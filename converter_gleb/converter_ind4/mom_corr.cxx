...
Double_t mom_corr::correct_pel_sim(double pe,double theta){  

Double_t   p0 = -0.00937397+theta*0.000462166+theta*theta*(-5.20498e-06);
Double_t   p1 = 0.021494+theta*(-0.00121626)+theta*theta*(1.39892e-05);
Double_t   p2 = -0.0110274+theta*0.000586353+theta*theta*(-6.90819e-06);

return pe-p0-p1*pe-p2*pe*pe; 
};
...
Double_t p0 = par[0] + theta*par[1] + theta*theta*par[2];
   Double_t p1 = par[3] + theta*par[4] + theta*theta*par[5];
   Double_t p2 = par[6] + theta*par[7] + theta*theta*par[8];
   Double_t p3 = par[9] + theta*par[10] + theta*theta*par[11];   
   Double_t p4 = par[12] + theta*par[13] + theta*theta*par[14];  
   Double_t p5 = par[15] + theta*par[16] + theta*theta*par[17]; 
   
   return  -1.*(p0+p1*pf+p2*pf*pf+p3*pf*pf*pf+p4*pf*pf*pf*pf+p5*pf*pf*pf*pf*pf)/pf;
