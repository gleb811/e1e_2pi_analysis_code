...
TH1I *hist_adc_off[12]; 
 ...
 Float_t Qfull = 0.;
  ...
  t21->SetDirectory(0);
        TH2F *beta_vs_p = new TH2F("beta_vs_p","beta_vs_p",100,0.,2.,100,0.,1.2);
...
//   cout << br_dc->GetLeaf("dc")->GetValue(0) << "\n";
 ...
 // cout << "grart = " << gpart << "\n";  
  ...
  //   cout << PdHit_PIp << "   " << sc_sect_local << "\n";
     if ((sc_sect_local==2)) {
     if ((PdHit_PIp<40))beta_vs_p->Fill(hadr_mom,beta);
     };
      if (hadr_mom <= E0) {
  ...
  //   t21->Write("", TObject::kOverwrite);
   //  outFile->Write();
     ...
     t21->Delete();
      ...
      TFile *outFile1;
     outFile1 = new TFile("beta_vs_p.root","recreate");
     outFile1->cd();
     beta_vs_p->Write("", TObject::kOverwrite);
     outFile1->Write();
     outFile1->Close();
