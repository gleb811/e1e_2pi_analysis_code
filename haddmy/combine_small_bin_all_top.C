void combine_small_bin_all_top() {
...
TFile *MyFile_out = new TFile("combine_small_bin_all_top3.root","RECREATE");
TFile *MyFile_data_out = new TFile("combine_data_small_bin_all_top3.root","RECREATE");

for (Int_t k=10; k<12;k++) {
Q2_bin = 0.425 + k*0.05; 
for (Int_t i=0; i<21;i++) { 
W_bin[i] = 1.3125+0.025*i; 
...
tmp_rec1[0]->Add(tmp_rec1[top]);
//tmp_gen1[0]->Add(tmp_gen1[top]);
tmp_data1[0]->Add(tmp_data1[top]);
tmp_empty1[0]->Add(tmp_empty1[top]);

tmp_rec2[0]->Add(tmp_rec2[top]);
//tmp_gen2[0]->Add(tmp_gen2[top]);
tmp_data2[0]->Add(tmp_data2[top]);
tmp_empty2[0]->Add(tmp_empty2[top]);

tmp_rec3[0]->Add(tmp_rec3[top]);
//tmp_gen3[0]->Add(tmp_gen3[top]);
tmp_data3[0]->Add(tmp_data3[top]);
tmp_empty3[0]->Add(tmp_empty3[top]);
...
qqq.str("");
qqq <<  "h_eff_1_" <<i;
tmp_eff1 = (THnSparseD*)tmp_rec1[0]->Clone(qqq.str().c_str());
tmp_eff1->Divide(tmp_gen1[0]);

qqq.str("");
qqq <<  "h_eff_2_" <<i;
tmp_eff2 = (THnSparseD*)tmp_rec2[0]->Clone(qqq.str().c_str());
tmp_eff2->Divide(tmp_gen2[0]);

qqq.str("");
qqq <<  "h_eff_3_" <<i;
tmp_eff3 = (THnSparseD*)tmp_rec3[0]->Clone(qqq.str().c_str());
tmp_eff3->Divide(tmp_gen3[0]);
...
eff1_bin = tmp_eff1->GetBin(bins);

rec2_bin = tmp_rec2[0]->GetBin(bins);
gen2_bin = tmp_gen2[0]->GetBin(bins);
eff2_bin = tmp_eff2->GetBin(bins);

rec3_bin = tmp_rec3[0]->GetBin(bins);
gen3_bin = tmp_gen3[0]->GetBin(bins);
eff3_bin = tmp_eff3->GetBin(bins);

if ((tmp_eff1->GetBinContent(eff1_bin)>0)){
eff_err = abs(tmp_gen1[0]->GetBinContent(gen1_bin) - tmp_rec1[0]->GetBinContent(rec1_bin));
eff_err = eff_err/(tmp_gen1[0]->GetBinContent(gen1_bin));
eff_err = eff_err/(tmp_gen1[0]->GetBinContent(gen1_bin));
eff_err = eff_err/(tmp_gen1[0]->GetBinContent(gen1_bin));

eff_err = eff_err*(tmp_rec1[0]->GetBinContent(rec1_bin));
eff_err = sqrt(eff_err);
tmp_eff1->SetBinError(eff1_bin,eff_err);
} else {
tmp_eff1->SetBinError(eff1_bin,0);
};

if ((tmp_eff2->GetBinContent(eff2_bin)>0)){
eff_err = abs(tmp_gen2[0]->GetBinContent(gen2_bin) - tmp_rec2[0]->GetBinContent(rec2_bin));
eff_err = eff_err/(tmp_gen2[0]->GetBinContent(gen2_bin));
eff_err = eff_err/(tmp_gen2[0]->GetBinContent(gen2_bin));
eff_err = eff_err/(tmp_gen2[0]->GetBinContent(gen2_bin));

eff_err = eff_err*(tmp_rec2[0]->GetBinContent(rec2_bin));
eff_err = sqrt(eff_err);
tmp_eff2->SetBinError(eff2_bin,eff_err);
} else {
tmp_eff2->SetBinError(eff2_bin,0);
};

if ((tmp_eff3->GetBinContent(eff3_bin)>0)){
eff_err = abs(tmp_gen3[0]->GetBinContent(gen3_bin) - tmp_rec3[0]->GetBinContent(rec3_bin));
eff_err = eff_err/(tmp_gen3[0]->GetBinContent(gen3_bin));
eff_err = eff_err/(tmp_gen3[0]->GetBinContent(gen3_bin));
eff_err = eff_err/(tmp_gen3[0]->GetBinContent(gen3_bin));

eff_err = eff_err*(tmp_rec3[0]->GetBinContent(rec3_bin));
eff_err = sqrt(eff_err);
tmp_eff3->SetBinError(eff3_bin,eff_err);
} else {
tmp_eff3->SetBinError(eff3_bin,0);
};
