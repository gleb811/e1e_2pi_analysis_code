void haddmy_high_w_pim_fine_theta_bin() {
...
TH1D *h_eff_err1[9][12];
TH1D *h_eff_err2[9][12];
TH1D *h_eff_err3[9][12];
TH1D *h_eff1[9][12];
TH1D *h_eff2[9][12];
TH1D *h_eff3[9][12];
THnSparseD *tmp_rec1;
...
THnSparseD *tmp_gen2;
THnSparseD *tmp_gen3;
TH1D *h_eff_err  = new TH1D("h_eff_err","h_eff_err",1400, -0.2,1.2);
...
W_bin[i] = 1.6875+0.025*i; 
qqq.str("");
qqq << "eff_err1_" << Q2_bin << "_" << W_bin[i];
h_eff_err1[i][k]  = new TH1D(qqq.str().c_str(),qqq.str().c_str(),1400, -0.2,1.2);
qqq.str("");
qqq << "eff_err2_" << Q2_bin << "_" << W_bin[i];
h_eff_err2[i][k]  = new TH1D(qqq.str().c_str(),qqq.str().c_str(),1400, -0.2,1.2);
qqq.str("");
qqq << "eff_err3_" << Q2_bin << "_" << W_bin[i];
h_eff_err3[i][k]  = new TH1D(qqq.str().c_str(),qqq.str().c_str(),1400, -0.2,1.2);
qqq.str("");
qqq << "eff1_" << Q2_bin << "_" << W_bin[i];
h_eff1[i][k]  = new TH1D(qqq.str().c_str(),qqq.str().c_str(),1000, 0.,1.);
qqq.str("");
qqq << "eff2_" << Q2_bin << "_" << W_bin[i];
h_eff2[i][k]  = new TH1D(qqq.str().c_str(),qqq.str().c_str(),1000, 0.,1.);
qqq.str("");
qqq << "eff3_" << Q2_bin << "_" << W_bin[i];
h_eff3[i][k]  = new TH1D(qqq.str().c_str(),qqq.str().c_str(),1000, 0.,1.);
};
};

for (Int_t j=0; j<=99; j++) {
qqq.str("");
qqq << "../out_high_w_fine_theta_binning" << j << ".root";
...
h_eff_err1[i][k]->Fill(eff_err/h_eff_1[i][k]->GetBinContent(eff1_bin));
h_eff1[i][k]->Fill(h_eff_1[i][k]->GetBinContent(eff1_bin));
} else {
...
h_eff_err2[i][k]->Fill(eff_err/h_eff_2[i][k]->GetBinContent(eff2_bin));
h_eff2[i][k]->Fill(h_eff_2[i][k]->GetBinContent(eff2_bin));
} else {
...
h_eff_err3[i][k]->Fill(eff_err/h_eff_3[i][k]->GetBinContent(eff3_bin));
h_eff3[i][k]->Fill(h_eff_3[i][k]->GetBinContent(eff3_bin));
} else {
...
TFile *MyFile_out = new TFile("high_w_pim_fine_theta_bin.root","UPDATE");
MyFile->cd();
...
h_gen_2[i][k]->Write();
h_gen_3[i][k]->Write();
h_eff_err1[i][k]->Write();
h_eff1[i][k]->Write();
h_eff_err2[i][k]->Write();
h_eff2[i][k]->Write();
h_eff_err3[i][k]->Write();
h_eff3[i][k]->Write();
/*qqq.str("");
qqq << "h_eff_1_" << i;
