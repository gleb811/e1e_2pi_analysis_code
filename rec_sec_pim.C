...
//Open output file
TFile *out_file = new TFile("out_rec_data.root","RECREATE");
///////  
...
//Creating  dirwcrories in output file
qqq.str("");
...
h_gen_1[i]->Scale(1/flux(i));
h_gen_2[i]->Scale(1/flux(i));
h_gen_3[i]->Scale(1/flux(i));

//h_data_1[i]->Scale(1/flux(i));
//h_data_2[i]->Scale(1/flux(i));
//h_data_3[i]->Scale(1/flux(i));
...
Int_t o_max =12;
Int_t p_max =12;
if ((i==0)||(i==1)) o_max = p_max = 8; 
if (i==2) o_max = p_max = 10;
...
Long64_t tmp_data1_bin = h_data_1[i]->GetBin(bins);
Long64_t tmp_data2_bin = h_data_2[i]->GetBin(bins);
Long64_t tmp_data3_bin = h_data_3[i]->GetBin(bins);

if (h_data_1[i]->GetBinContent(tmp_data1_bin) > 0.) {
h_data_1[i]->SetBinError(tmp_data1_bin,sqrt(h_data_1[i]->GetBinContent(tmp_data1_bin)));
} else {
h_data_1[i]->SetBinError(tmp_data1_bin,0.);
};

if (h_data_2[i]->GetBinContent(tmp_data2_bin) > 0.) {
h_data_2[i]->SetBinError(tmp_data2_bin,sqrt(h_data_2[i]->GetBinContent(tmp_data2_bin)));
} else {
h_data_2[i]->SetBinError(tmp_data2_bin,0.);
};

if (h_data_3[i]->GetBinContent(tmp_data3_bin) > 0.) {
h_data_3[i]->SetBinError(tmp_data3_bin,sqrt(h_data_3[i]->GetBinContent(tmp_data3_bin)));
} else {
h_data_3[i]->SetBinError(tmp_data3_bin,0.);
};

Double_t err1,err2;

if (h_eff_1[i]->GetBinContent(tmp_eff1_bin) > 0.) {

err1 = (h_rec_1[i]->GetBinContent(tmp_rec1_bin))*(h_rec_1[i]->GetBinContent(tmp_rec1_bin));
err1 = err1/(h_gen_1[i]->GetBinContent(tmp_gen1_bin));
err1 = err1 + (h_rec_1[i]->GetBinContent(tmp_rec1_bin));
err1 = sqrt(err1);
err1 = err1/(h_gen_1[i]->GetBinContent(tmp_gen1_bin));
...
err1 = (h_rec_2[i]->GetBinContent(tmp_rec2_bin))*(h_rec_2[i]->GetBinContent(tmp_rec2_bin));
err1 = err1/(h_gen_2[i]->GetBinContent(tmp_gen2_bin));
err1 = err1 + (h_rec_2[i]->GetBinContent(tmp_rec2_bin));
err1 = sqrt(err1);
err1 = err1/(h_gen_2[i]->GetBinContent(tmp_gen2_bin));
...
if (h_eff_3[i]->GetBinContent(tmp_eff3_bin) > 0.) {

err1 = (h_rec_3[i]->GetBinContent(tmp_rec3_bin))*(h_rec_3[i]->GetBinContent(tmp_rec3_bin));
err1 = err1/(h_gen_3[i]->GetBinContent(tmp_gen3_bin));
err1 = err1 + (h_rec_3[i]->GetBinContent(tmp_rec3_bin));
err1 = sqrt(err1);
err1 = err1/(h_gen_3[i]->GetBinContent(tmp_gen3_bin));
...
leg = new TLegend(0.4,0.965,0.9,1.0); 
leg->SetNColumns(2);
leg->SetFillStyle(0);
...
TH1D *data_tmp = h_data_1[i]->Projection(0,"");
 TH1D *model_tmp = model_1[i]->Projection(0,"") ; 
// data_tmp->Scale((model_tmp->GetMaximum())/(data_tmp->GetMaximum()));
draw_1d_hist(1,data_tmp,"","h1prj_inv_m_pip_p_1_","Yield","m_{#pi^{+}p}  (GeV)",1,"e1P","mass",i);
qqq.str("");


 TH1D *rec_tmp = h_rec_1[i]->Projection(0,"");
 TH1D *model_tmp = model_1[i]->Projection(0,"") ; 
 rec_tmp->Scale(((h_data_1[i]->Projection(0,""))->Integral())/(rec_tmp->Integral()));

draw_1d_hist(1,rec_tmp,qqq.str(),"h1prj_inv_m_pip_p_1_gen","Yield","m_{#pi^{+}p} (GeV)",4,"e1AP same","mass",i);
qqq.str("");
...
// pi+ pi- mass
 ...
 data_tmp = h_data_1[i]->Projection(1,"");
 model_tmp = model_1[i]->Projection(1,"") ; 
// data_tmp->Scale((model_tmp->GetMaximum())/(data_tmp->GetMaximum()));
draw_1d_hist(2,data_tmp,"","h1prj_inv_m_pip_pim_1_","Yield","m_{#pi^{+}#pi^{-}} (GeV)",1,"e1P","mass",i);

 rec_tmp = h_rec_1[i]->Projection(1,"");
 model_tmp = model_1[i]->Projection(1,"") ; 
 rec_tmp->Scale(((h_data_1[i]->Projection(1,""))->Integral())/(rec_tmp->Integral()));

draw_1d_hist(2,rec_tmp,"","h1prj_inv_m_pip_pim_1_gen","Yield","m_{#pi^{+}#pi^{-}} (GeV)",4,"e1AP same","mass",i);
...
// pi- proton mass
 ...
 data_tmp = h_data_3[i]->Projection(0,"");
 model_tmp = model_3[i]->Projection(0,"") ; 
// data_tmp->Scale((model_tmp->GetMaximum())/(data_tmp->GetMaximum()));
draw_1d_hist(3,data_tmp,"","h3prj_inv_m_pim_p_1_","Yield","m_{#pi^{-}p} (GeV)",1,"e1P","mass",i);
...
//draw_1d_hist(3,h_cr_sect_noemptcells_3[i]->Projection(0,""),"","h3prj_inv_m_pim_p_1_noempty","d#sigma/dM, #mubn/GeV","m_{#pi^{-}p}, GeV",2,"e1AP same","mass",i);
 ...
 rec_tmp = h_rec_3[i]->Projection(0,"");
 model_tmp = model_3[i]->Projection(0,"") ; 
 rec_tmp->Scale(((h_data_3[i]->Projection(0,"")->Integral()))/(rec_tmp->Integral()));
 
draw_1d_hist(3,rec_tmp,"","h3prj_inv_m_pim_p_1_gen","Yield","m_{#pi^{-}p} (GeV)",4,"e1AP same","mass",i); 
...
// theta proton
 ...
 data_tmp = h_data_1[i]->Projection(2,"");
 model_tmp = model_1[i]->Projection(2,"") ; 
// data_tmp->Scale((model_tmp->GetMaximum())/(data_tmp->GetMaximum()));

draw_1d_hist(4,data_tmp,"","h1prj_th_P_","Yield/d(-cos#theta)","#theta_{p'} in c.m. (deg)",1,"e1P","theta",i);

 rec_tmp = h_rec_1[i]->Projection(2,"");
 model_tmp = model_1[i]->Projection(2,"") ; 
 rec_tmp->Scale(((h_data_1[i]->Projection(2,""))->Integral())/(rec_tmp->Integral()));

draw_1d_hist(4,rec_tmp,"","h1prj_th_P_gen","Yield/d(-cos#theta)","#theta_{p'} in c.m. (deg)",4,"e1AP same","theta",i);
...
// theta pi-
 ...
 data_tmp = h_data_2[i]->Projection(2,"");
 model_tmp = model_2[i]->Projection(2,"") ; 
// data_tmp->Scale((model_tmp->GetMaximum())/(data_tmp->GetMaximum()));
draw_1d_hist(5,data_tmp,"","h1prj_th_PIm_","Yield/d(-cos#theta)","#theta_{#pi^{-}} in c.m. (deg)",1,"e1P","theta",i);


 rec_tmp = h_rec_2[i]->Projection(2,"");
 model_tmp = model_2[i]->Projection(2,"") ; 
 rec_tmp->Scale(((h_data_2[i]->Projection(2,""))->Integral())/(rec_tmp->Integral()));

draw_1d_hist(5,rec_tmp,"","h1prj_th_PIm_gen","Yield/d(-cos#theta)","#theta_{#pi^{-}} in c.m. (deg)",4,"e1AP same","theta",i);
...
data_tmp = h_data_3[i]->Projection(2,"");
 model_tmp = model_3[i]->Projection(2,"") ; 
// data_tmp->Scale((model_tmp->GetMaximum())/(data_tmp->GetMaximum()));
draw_1d_hist(6,data_tmp,"","h1prj_th_PIp_","Yield/d(-cos#theta), #mubn/rad","#theta_{#pi^{+}} in c.m. (deg)",1,"e1P","theta",i);

 rec_tmp = h_rec_3[i]->Projection(2,"");
 model_tmp = model_3[i]->Projection(2,"") ; 
 rec_tmp->Scale(((h_data_3[i]->Projection(2,""))->Integral())/(rec_tmp->Integral()));
 
 draw_1d_hist(6,rec_tmp,"","h1prj_th_PIp_gen","Yield/d(-cos#theta)","#theta_{#pi^{+}} in c.m. (deg)",4,"e1AP same","theta",i); 
...
// alpha proton
 ...
 data_tmp = h_data_1[i]->Projection(4,"");
 model_tmp = model_1[i]->Projection(4,"") ; 
// data_tmp->Scale((model_tmp->GetMaximum())/(data_tmp->GetMaximum()));
draw_1d_hist(7,data_tmp,"","h1prj_alpha_PIpPIm_pipf_","Yield","#alpha_{p'} (deg)",1,"e1P","alpha",i);


 rec_tmp = h_rec_1[i]->Projection(4,"");
 model_tmp = model_1[i]->Projection(4,"") ; 
 rec_tmp->Scale(((h_data_1[i]->Projection(4,""))->Integral())/(rec_tmp->Integral()));

draw_1d_hist(7,rec_tmp,"","h1prj_alpha_PIpPIm_pipf_gen","Yield","#alpha_{p'} (deg)",4,"e1AP same","alpha",i);
...
//draw_1d_hist(8,model_2[i]->Projection(4,""),"","model_alpha_pip_","Yield","alpha_{#pi^{-}} (deg)",3,"e1","alpha",i);
...
// alpha pi+
 ...
 data_tmp = h_data_2[i]->Projection(4,"");
 model_tmp = model_2[i]->Projection(4,"") ; 
// data_tmp->Scale((model_tmp->GetMaximum())/(data_tmp->GetMaximum()));
 
draw_1d_hist(8,data_tmp,"","h2prj_alpha_PPIp_piPIm_","Yield","#alpha_{#pi^{-}} (deg)",1,"e1P","alpha",i);


 rec_tmp = h_rec_2[i]->Projection(4,"");
 model_tmp = model_2[i]->Projection(4,"") ; 
 rec_tmp->Scale(((h_data_2[i]->Projection(4,""))->Integral())/(rec_tmp->Integral()));
 
 draw_1d_hist(8,rec_tmp,"","h2prj_alpha_PPIp_piPIm_gen","Yield","#alpha_{#pi^{-}} (deg)",4,"e1AP same","alpha",i);
...
// alpha pi-
 ...
 data_tmp = h_data_3[i]->Projection(4,"");
 model_tmp = model_3[i]->Projection(4,"") ; 
// data_tmp->Scale((model_tmp->GetMaximum())/(data_tmp->GetMaximum()));
draw_1d_hist(9,data_tmp,"","h3prj_alpha_PPIm_piPIp_","Yield","#alpha_{#pi^{+}} (deg)",1,"e1P","alpha",i);


 rec_tmp = h_rec_3[i]->Projection(4,"");
 model_tmp = model_3[i]->Projection(4,"") ; 
 rec_tmp->Scale(((h_data_3[i]->Projection(4,""))->Integral())/(rec_tmp->Integral()));

draw_1d_hist(9,rec_tmp,"","h3prj_alpha_PPIm_piPIp_gen","Yield","#alpha_{#pi^{+}} (deg)",4,"e1AP same","alpha",i); 
// alpha pi-  no empty cells filling
...
qqq.str("");
qqq << "Q2_0" << ((10*(4 + qq2/2) + (qq2%2)*5)/10.)*(1+(qq2%2)*9) << "_0" << ((10*(4 + (qq2+1)/2) + ((qq2+1)%2)*5)/10.)*(1+((qq2+1)%2)*9) << "/" << "w_" << W_bin[i] << ".pdf" ;
if (qq2 == 11) {
qqq.str("");
